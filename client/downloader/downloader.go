...
func DownloadFile(filename string, downloadDir string) error {
	...
	f, err := os.OpenFile(filePath, os.O_WRONLY|os.O_CREATE, 0666)//f为创建的文件
	if err != nil {
	...
	_, err = io.Copy(f, reader)//将reader写入f
	if err != nil {
		return err
	}
	//文件校验
